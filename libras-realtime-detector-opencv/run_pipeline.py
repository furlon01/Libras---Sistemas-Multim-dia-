...
Roda a sessão de gravação inteira, do começo ao fim, em um único comando:
  ...
  1. Abre a câmera (python -m opencvapp) -- ESSA ETAPA É BLOQUEANTE DE PROPÓSITO:
     ...
     Isso é esperado -- é o loop de captura rodando ao vivo. Ao apertar
     ESC, o próprio módulo opencvapp já monta o vídeo final sincronizado
     (session_video_fixed.mp4), quadro a quadro, com timing exato -- não
     é mais preciso reencodar/corrigir o FPS depois.
  2. Assim que essa etapa termina, o script continua automaticamente:
...
def main():
    print("=" * 60)
    print("ETAPA 1/4: gravando a sessão da câmera")
    print("O terminal vai ficar 'preso' aqui até você apertar ESC")
    print("com a janela da câmera em foco -- isso é esperado.")
    print("Ao encerrar, o vídeo final sincronizado já é montado")
    print("automaticamente (session_video_fixed.mp4).")
    print("=" * 60)
    ...
    video_path = os.path.join(ROOT, "session_video_fixed.mp4")

    print("=" * 60)
    print("ETAPA 2/4: convertendo legenda pra WebVTT")
    print("=" * 60)
    run([sys.executable, os.path.join(ROOT, "tv3_pipeline", "srt_to_vtt.py"),
         "legendas.srt", os.path.join(WEB_DIR, "legendas.vtt")])

    print("=" * 60)
    print("ETAPA 3/4: gerando a trilha de audiodescrição (TTS)")
    print("=" * 60)
    run([sys.executable, os.path.join(ROOT, "tv3_pipeline", "generate_audiodesc_track.py"),
         "legendas.srt", video_path, "audiodescricao.m4a"])

    print("=" * 60)
    print("ETAPA 4/4: empacotando em MPEG-DASH e subindo o servidor")
    print("=" * 60)
    run([sys.executable, os.path.join(ROOT, "tv3_pipeline", "package_dash.py"),
         video_path, "audiodescricao.m4a", WEB_DIR])
